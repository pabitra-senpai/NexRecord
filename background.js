 ...
 * NOTE: tabCapture is now handled directly in the popup (offscreen-free MV3 pattern).
 */
...
chrome.commands.onCommand.addListener((command) => {
  if (command === 'toggle-recording') {
    if (state.status === RECORDING_STATE.IDLE) {
      // Open popup — user must click record there (tabCapture needs popup context)
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  switch (message.type) {
    ...
    // Popup tells us recording started (popup owns the MediaRecorder)
    ...
    // Popup asks for the active tab info
