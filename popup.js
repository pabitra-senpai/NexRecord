...
async function init() {
  ...
  // Listen for background state pushes and commands
  chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
    ...
    } else if (msg.type === 'COMMAND_PAUSE') {
      if (currentStatus === 'recording') pauseRecording();
      sendResponse({ success: true });
    } else if (msg.type === 'COMMAND_RESUME') {
      if (currentStatus === 'paused') resumeRecording();
      sendResponse({ success: true });
    }
  });
}
...
async function saveRecording() {
  ...
  try {
    ...
    toast('Error saving file. Check storage.', 'error');
    setStatus('idle', 'Error saving recording.');
  }
}
