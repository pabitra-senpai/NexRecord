...
async function startRecording() {
  ...
  try {
    const res = await chrome.runtime.sendMessage({ type: MSG.START_RECORDING, options });

    if (!res.success) {
      setStatus('idle', res.error || 'Failed to start recording.');
      toast(res.error || 'Failed to start.', 'error');
      return;
    }

    // Use the stream ID to get the MediaStream in the popup context
    const constraints = {
      audio: {
        mandatory: {
    ...
    setStatus('idle', err.message || 'Microphone/tab access denied.');
    toast(err.message || 'Could not access tab audio.', 'error');
    // Tell background to reset
    chrome.runtime.sendMessage({ type: MSG.STOP_RECORDING }).catch(() => {});
  }
}
